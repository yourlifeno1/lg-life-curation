...
# 2. [매뉴얼 기준] 서울 115개 공식 지점 매핑 (대표 지점 위주)
CITY_POINTS = [
...
def fetch_moving_total(lawd_cd, year_month):
    """아파트+연립다세대+오피스텔 합산 로직 복구 (20건 이상 산출)"""
    total = 0
...
if loc:
    ...
    try:
        ...
        # 성별 및 연령대 비중 추출
        fem = float(root.find('.//FEMALE_PPLTN_RATE').text)
        pop_info = f"{'여성' if fem > 50 else '남성'} 타겟 비중 높음"
    ...
    st.divider()
    st.success(f"현재 {target['name']} 주변은 **{pop_info}** 상태입니다. 현장 영업 시 참고하세요!")
else:
    st.info("🛰️ 실시간 GPS 좌표를 수신하여 기상도를 분석 중입니다...")
