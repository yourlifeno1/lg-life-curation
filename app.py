...
import pandas as pd  # [수정완료] 라이브러리 호출 정상화
import requests
import json
...
# 3. 6개 End Point 통합 이사 지수 데이터 호출
...
def fetch_realtime_traffic():
    ...
    try:
        ...
        # 상권 활력도 점수만 심야 시간대(22-07) 가중치 적용
...
if loc:
    ...
    # 데이터 로드
    this_month_val = datetime.now().strftime("%Y%m")
    last_month_val = (datetime.now().replace(day=1) - timedelta(days=1)).strftime("%Y%m")
    
    real_traffic, vitality_score = fetch_realtime_traffic()
    cnt_now = fetch_robust_moving_data(lawd_cd, this_month_val)
    cnt_prev = fetch_robust_moving_data(lawd_cd, last_month_val)
    ...
    # 분석 버튼
    if st.button(f"🚀 {current_dong} 정밀 리포트 생성 및 전송"):
        with st.status("데이터 분석 중..."):
            payload = {
                "region": current_dong, "weather": vitality_score, "move_idx": int(diff_pct),
                "care_score": 85, "care_reason": f"{datetime.now().month}월 현재 {cnt_now}건 (전월 {cnt_prev}건 대비)",
                "as_reason": f"실시간 유동인구 {real_traffic}명 기반",
                "recommend_prod": "이사 가전 큐레이션", "issue": f"{gu_name} 트렌드 반영"
            }
    ...
    with c1:
        # 상단에 점수, 하단에 실제 유동인구 수 표시
        st.metric("상권 활력 점수", f"{vitality_score}점", f"실시간 유동 {real_traffic}명")
        
    with c2:
        # 상단에 현재월 거래건수, 하단에 전월 대비 증감률 표시
    ...
    st.info(f"**이사 분석:** 현재 {gu_name} 지역은 {cnt_now}건의 부동산 거래가 감지되었습니다. 전월({cnt_prev}건) 대비 흐름을 추적 중입니다.")
    st.warning(f"**상권 분석:** 현재 시간 가중치가 적용된 상권 활력도는 **{vitality_score}점**입니다.")

else:
    st.info("🛰️ GPS 좌표를 수신하고 있습니다. 브라우저의 위치 권한을 확인해주세요.")
