...
def advanced_kor_to_num(text):
    ...
    pattern = r'([일이삼사오육칠팔구십백천만억한두세네다섯여섯일곱여덟아홉열스무서른마흔쉰예순일흔여든아흔\s]+)\s*(센티|센치|밀리|미터|인치|평|자|살|세|만원|톤|리터)'
...
if final_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("고객이 반응하는 중..."):
            ...
            - (정보 공개의 단계): 초기에는 단답형으로 대응하되, 매니저가 열린 질문을 던지거나 공감을 잘해준다면 당신의 고민이나 상황을 조금 더 상세히 털어놓으세요.
            - (언어): 전문 용어는 피하고, "그건 좀 비싸네요", "자리가 좁을 것 같아요" 같은 일상적인 표현만 쓰세요.
            - (말투): "~인 것 같아요", "~인가요?", "~네요" 처럼 고객의 입장에서 말하세요. (판매원 말투 절대 금지)
            - 사용자가 가격을 언급하면 반드시 결제 방식에 대한 구체적인 부담감을 표현하며 거절 시그널을 보내세요.
            - 스테이지 종료 2회 전에는 반드시 이탈 예고('가봐야겠다', '다음에 오겠다')를 하여 사용자를 압박하세요.
            """
            ...
            4. (답변 길이의 유연성): 
               - 매니저가 단답형 질문을 하면 1~2문장으로 짧게 답하세요.
               - 매니저가 당신의 라이프스타일을 묻는 '열린 질문'을 하거나 공감을 잘해준다면, 3~5문장 내외로 당신의 상황을 구체적으로 설명하세요.
            5. 절대로 매니저보다 먼저 제품 스펙을 언급하거나 판매원처럼 제안하지 마세요.
            """       
        ...
        if st.session_state.show_report:
            ...
            if "feedback_result" not in st.session_state:
                with st.spinner("세일즈 마스터 코치가 대화를 엄격하게 분석 중입니다..."):
                    ...
                    if menu_key == "라포형성":
                        criteria = """
                        - **스몰토크 및 경계 해제**: 일상적인 주제로 고객의 마음을 열고 대화의 주도권을 확보했는가?
                        - **잠재적 니즈 자극 (구독 빌드업)**: 대화 중 자연스럽게 '전문적 케어'나 '구독의 가치'를 언급하며 향후 제안을 위한 밑거름을 깔았는가? (미언급 시 조언 필수)
                        - **첫인사 및 매너**: 정중하고 친근한 태도로 고객이 이 공간을 편안하게 느끼도록 유도했는가?
                        """
                    elif menu_key == "니즈파악":
                        criteria = """
                        - **라이프스타일 발굴**: 고객의 가전 사용 습관을 파악하며 '관리의 번거로움'이나 '비용 부담' 등의 페인 포인트를 찾아냈는가?
                        - **구독 명분 만들기**: 고객의 답변을 활용해 왜 '구독 서비스'가 고객에게 최적의 대안인지 연결 고리를 만들었는가? (미연결 시 조언 필수)
                        - **질문의 기술**: 고객이 스스로 불편함을 이야기하도록 열린 질문을 적절히 사용했는가?
                        """
                    elif menu_key == "클로징":
                        criteria = """
                        - **구독 전환 골든타임 (핵심)**: 가격 저항 시 즉시 '가전 구독'을 대안으로 제시하여 기회를 전환했는가? (미제안 시 강력 감점)
                        - **거절 극복 소구력**: 가격 및 관리 고민에 대해 구독의 3대 혜택(비용절감, 케어, 무상수리)을 논리적으로 설명했는가?
                        - **클로징 결정력**: 망설이는 고객에게 구독을 통한 합리적 결정을 확신 있게 제안했는가?
                        """
                    else: # VOC 상황 (매장/전화)
                        criteria = """
                    ...
                    [언어 규칙]
                    1. 모든 설명과 문장은 한글 작성을 원칙으로 하되 영문 약어는 허용합니다.
