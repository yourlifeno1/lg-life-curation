...
if final_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("고객이 반응하는 중..."):
            ...
            if has_companion:
                role_instruction = f"""
                - **제품 일관성**: 현재 보고 있는 제품은 '{data.get('product')}'입니다. 고객과 동반인이 서로 다른 제품을 말하지 않도록 주의하세요.
                - **1인 2역 대화**: 반드시 [고객]과 [동반인] 태그를 사용하되, 한 인물이 연속해서 말할 때는 태그를 한 번만 쓰세요.
                - **가독성**: [고객]과 [동반인]의 대사 뭉치 사이에는 반드시 줄바꿈을 두 번 하세요.
                - **예시**:
                  [고객] (에어컨을 보며) "이거 소음은 어떤가요?"
                  (한 줄 띄고)
                  [동반인] (옆에서 거들며) "맞아요, 밤에 켜둘 거라 조용해야 해요."
                """
            else:
                role_instruction = f"""
