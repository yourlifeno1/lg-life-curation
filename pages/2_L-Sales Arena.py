...
if len(chat_only) > 1:
    ...
    with btn_col2:
        if st.button("📊 상담 종료 및 코칭 리포트 보기", use_container_width=True):
            
            # 대화가 너무 짧은 경우 (최소 4번의 대화 왕복 기준)
            if len(chat_only) < 4:
                ...
                with st.spinner("세일즈 마스터 코치가 대화를 엄격하게 분석 중입니다..."):
                    ...
                    훈련생의 발전을 위해 **매우 날카롭고 객관적으로** 평가하되, 아래의 언어 규칙을 반드시 지키십시오.

                    [언어 및 단위 규칙]
                    - **순수 한글 사용**: 모든 답변에서 영어 알파벳(A-Z, a-z) 사용을 절대 금지합니다.
                    - **단위 한글화**: 측정 단위는 기호 대신 반드시 한글 명칭으로 작성하세요.
                      * 예시: 86인치, 24킬로그램, 200와트, 500그램, 900리터 등
                    - **외래어 처리**: 꼭 필요한 외래어는 한글 발음으로만 적으십시오. (예: 베스트, 피드백, 모니터링)

                    [코칭 원칙]
                    1. **객관적 평가**: 대화가 짧거나 알맹이가 없다면 무조건 칭찬하지 마세요. 부족한 점은 단호하게 지적하십시오.
                    2. **NCS 기반 분석**: 라포 형성/니즈 파악/해결책 제시 능력을 냉정하게 분석하세요.
                    3. **게임 요소**: 성과가 좋지 않으면 낮은 등급의 칭호를 부여하세요 (예: 아직은 세일즈 새내기).
                    
                    [리포트 구성 요소]
                    - **🏆 오늘의 세일즈 칭호**: (현재 실력에 걸맞은 솔직한 별명)
                    - **⭐ 항목별 스킬 점수**: 기준에 미달하면 과감히 낮은 점수(1~2점)를 부여하십시오.
                    - **✨ 오늘의 빛나는 순간**: 센스 있는 문장이 없었다면 "발견되지 않음"이라고 적고 이유를 설명하세요.
                    - **💡 한 끗 차이 레벨업**: 구체적이고 실전적인 개선 방안 2가지를 제시하세요.
                    """
